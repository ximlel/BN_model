...
double inflow_Riemann_solver_exact(double * u_star, double * h_star, const double g, double q_star, double u_R, double h_R, int * CRW, double tol, int N)
{
    ...
    *u_star = q_star/h_R;        
    ...
    k1 = -q_star/(*h_star)*(*h_star);//the (h,u)-tangent slope on I1 at (u_star,h_star), i.e. [du/dh](h_L)
    ...
    h_INT = (k1*(*h_star) - k3*h_R - (*u_star) + u_R) / (k1 - k3);
    if(h_INT <= 0)
	h_INT = *h_star;
    h_INT = 0.5*h_INT;
...
{
    double h_R, s_h_R, u_R, s_u_R, B, s_B_L, s_B_R, g, eps;
    q_star = input[0];
    h_R = input[1];
    s_h_R = input[2];
    u_R = input[3];
    ...
    //=========non-acoustic case==========
    Riemann_solver_exact(&u_star, &h_star, g, q_star, u_R, h_R, CRW, eps, 50);
