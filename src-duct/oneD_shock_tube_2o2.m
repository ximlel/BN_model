clear;
clc;
%1D shock_tube by GRP Schemes for duct-flow model
%state constant
global gama;
gama=1.23;
global ep;
...
while Time<Tend && isreal(Time)
    ...
    for i=3:N-1
        if abs(A(i+1)-A(i))<ep && abs(A(i+2)-A(i+1))<ep && abs(A(i)-A(i-1))<ep
            dlo(i) =minmod(Alpha_GRP*(lo_R(i)-lo_R(i-1))/d_x,(lo_L(i+1)-lo_L(i-1))/2.0/d_x,Alpha_GRP*(lo_L(i+1)-lo_L(i))/d_x);
            du(i)  =minmod(Alpha_GRP*(u_R(i) -u_R(i-1) )/d_x,(u_L(i+1) -u_L(i-1) )/2.0/d_x,Alpha_GRP*(u_L(i+1) -u_L(i) )/d_x);
            dp(i)  =minmod(Alpha_GRP*(p_R(i) -p_R(i-1) )/d_x,(p_L(i+1) -p_L(i-1) )/2.0/d_x,Alpha_GRP*(p_L(i+1) -p_L(i) )/d_x);
%             dlo(i) =minmod(Alpha_GRP*(lo_R(i)-lo_R(i-1))/d_x,(W_int(1,i+1)-W_int(1,i))/1.0/d_x,Alpha_GRP*(lo_L(i+1)-lo_L(i))/d_x);
%             du(i)  =minmod(Alpha_GRP*(u_R(i) -u_R(i-1) )/d_x,(W_int(2,i+1)-W_int(1,i))/1.0/d_x,Alpha_GRP*(u_L(i+1) -u_L(i) )/d_x);
%             dp(i)  =minmod(Alpha_GRP*(p_R(i) -p_R(i-1) )/d_x,(W_int(3,i+1)-W_int(1,i))/1.0/d_x,Alpha_GRP*(p_L(i+1) -p_L(i) )/d_x);
        end
    end
    ...
    for i=1:N+1
        %flux on the boundary of i-1 and i
         ...
         elseif i==N+1
            ...
            [F(:,i),W_int(:,i)]=GRP_solver(lo_R(i-1)+0.5*d_x*dlo(i-1),lo_L(i)-0.5*d_x*dlo(i),dlo(i-1),dlo(i),u_R(i-1)+0.5*d_x*du(i-1),u_L(i)-0.5*d_x*du(i),du(i-1),du(i),p_R(i-1)+0.5*d_x*dp(i-1),p_L(i)-0.5*d_x*dp(i),dp(i-1),dp(i),A(i),A(i),0.0,0.0,gama,d_t);
         end
    end
...
plot(x_min:d_x:x_max-d_x,p,col,'MarkerSize',6);%,col,'LineWidth',1.0);
%xlabel('Position','FontWeight','bold');
...
plot(x_min:d_x:x_max-d_x,eta,col,'MarkerSize',6);%,col,'LineWidth',1.0);
%xlabel('Position','FontWeight','bold');
