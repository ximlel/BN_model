...
void primitive_comp(double * U, struct U_var * U_L, struct U_var * U_R, double phi_sL, double phi_sR, double area_L, double area_R)
{   
	...
	double phi_g = 1.0-phi_s;
	double U1=U[0], U2=U[1], U3=U[2], U4=U[3], U5=U[4], U6=U[5];
	...
	double lo_gL= (U1-area_R*phi_gR*lo_gR)/area_L/phi_gL;
	double u_gL = (U2-area_R*phi_gR*lo_gR*u_gR)/(U1-area_R*phi_gR*lo_gR);
	double p_gL = p_gR/pow(lo_gR,gama_g)*pow((U1-area_R*phi_gR*lo_gR)/area_L/phi_gL,gama_g);
	double p_sL = ((U6-0.5*phi_s*lo_s*pow(u_s,2.0))*(gama_s-1)-area_R*phi_sR*p_sR)/area_L/phi_sL;
	double lo_sL= lo_s;
	double lo_sR= lo_s;
	double u_sL = u_s;
	double u_sR = u_s;
