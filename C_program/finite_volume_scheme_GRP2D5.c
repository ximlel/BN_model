		...
		for(int j = 1; j < n_x-1; ++j)
			{
				...
				jL=j-1;
				jR=j+1;
				if (fabs((Z_lC[i][jR]-Z_lC[i][jL])/config[10]/2)>eps || fabs((Z_lC[iR][j]-Z_lC[iL][j])/config[11]/2)>eps)
					for (int k = -HN; k < HN; k++)
						{
							if (i+k >= 0 && i+k < n_y)
								{
...
void finite_volume_scheme_GRP2D(struct flu_var * FV, const struct mesh_var * mv, const char * scheme, const char * problem)
{
	...
	init_data_2(Z_l_2D, U_RHO_g_2D, U_U_g_2D, U_V_g_2D, U_E_g_2D, U_RHO_l_2D, U_U_l_2D, U_V_l_2D, U_E_l_2D);
	for(int i = 0; i < n_y; ++i)
		for(j = 0; j < n_x; ++j)
			{
				if (j >=n_x/2 && i >=n_y/2)
			...
			if (t_all >= plot_t)
				{
					for(int i = 0; i < n_y; ++i)
						for(j = 0; j < n_x; ++j)
							{
								FV->RHO[i*n_x+j]=RHO_lC[i][j];
								FV->U[i*n_x+j]  =U_lC[i][j];
								FV->V[i*n_x+j]  =V_lC[i][j];
								FV->P[i*n_x+j]  =P_lC[i][j];
						...
						for(j = 0; j < n_x; ++j)
							{
								...
								//								printf("\n%2d,%2d,%lf,%lf,%lf,%lf,%lf,%lf,%lf\n",i,j,U[0],U[1],U[2],U[3],U[4],U[5],U[6]);
								//								printf("\n%2d,%2d,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf\n",i,j,RHO_gC[i][j],U_gC[i][j],V_gC[i][j],P_gC[i][j],RHO_lC[i][j],U_lC[i][j],V_lC[i][j],P_lC[i][j]);
							}
				}
			else if (stop_step == 2)
				{		
						...
						for(j = 0; j <=n_x; ++j)
							{
								...
								stag_RHO_F_lx[i][j] = U_R[i][j].lo_s*U_R[i][j].u_s;
								if (U_R[i][j].u_s > 0.0)
									stag_ZRHO_F_lx[i][j] = U_R[i][j].lo_s*U_R[i][j].u_s*z_lL;
								else
									stag_ZRHO_F_lx[i][j] = U_R[i][j].lo_s*U_R[i][j].u_s*z_lR;
							}
					for(int i = 0; i < n_y; ++i)
						for(j = 0; j < n_x; ++j)
							{
								...
								if (j == n_x-1)
									...
									jp1 = j+1;
								RHO_l_cell  = 0.25*(U_R[i][j].lo_s+U_R[ip1][j].lo_s+U_L[i][jp1].lo_s+U_L[ip1][jp1].lo_s);
								ZRHO_l_cell = 0.25*(ZRHO_lC[i][j]+ZRHO_lC[ip1][j]+ZRHO_lC[i][jp1]+ZRHO_lC[ip1][jp1]);
								RHO_l_cell  -=tau*0.5*(stag_ZRHO_F_lx[i][j+1]+stag_ZRHO_F_lx[ip1][j+1]-stag_ZRHO_F_lx[i][j]-stag_ZRHO_F_lx[ip1][j])/dx;
								ZRHO_l_cell -=tau*0.5*(stag_ZRHO_F_lx[i][j+1]+stag_ZRHO_F_lx[ip1][j+1]-stag_ZRHO_F_lx[i][j]-stag_ZRHO_F_lx[ip1][j])/dx;
						...
						for(j = 0; j < n_x; ++j)
							{
								...
								U[6] = RHO_V_lC[i][j];
								U[7] = E_lC[i][j]-0.5*RHO_V_lC[i][j]*RHO_V_lC[i][j]/ZRHO_lC[i][j];
								primitive_comp(U, &U_L[i][j], &U_R[i][j], phi_sL[i][j], phi_sR[i][j], area_L, area_R);
						...
						for(int i = 0; i <= n_y; ++i)
							{
								...
								stag_RHO_F_ly[i][j] = U_R[i][j].lo_s*U_R[i][j].u_s;
								if (U_R[i][j].u_s > 0.0)
									stag_ZRHO_F_ly[i][j] = U_R[i][j].lo_s*U_R[i][j].u_s*z_lL;
								else
									stag_ZRHO_F_ly[i][j] = U_R[i][j].lo_s*U_R[i][j].u_s*z_lR;
							}
					for(j = 0; j < n_x; ++j)
						for(int i = 0; i < n_y; ++i)
							{
								...
								if (j == n_x-1)
									...
									jp1 = j+1;
								RHO_l_cell  = 0.25*(U_R[i][j].lo_s+U_L[ip1][j].lo_s+U_R[i][jp1].lo_s+U_L[ip1][jp1].lo_s);
								ZRHO_l_cell = 0.25*(ZRHO_lC[i][j]+ZRHO_lC[ip1][j]+ZRHO_lC[i][jp1]+ZRHO_lC[ip1][jp1]);
								RHO_l_cell  -=tau*0.5*(stag_ZRHO_F_ly[i+1][j]+stag_ZRHO_F_ly[i+1][jp1]-stag_ZRHO_F_ly[i][j]-stag_ZRHO_F_ly[i][jp1])/dy;
								ZRHO_l_cell -=tau*0.5*(stag_ZRHO_F_ly[i+1][j]+stag_ZRHO_F_ly[i+1][jp1]-stag_ZRHO_F_ly[i][j]-stag_ZRHO_F_ly[i][jp1])/dy;
								Z_lC[i][j]   =ZRHO_l_cell/RHO_l_cell;
							}
					for(j = 0; j < n_x; ++j)
						for(int i = 0; i < n_y; ++i)
						...
						for(int i = 0; i < n_y; ++i)
							{
								if(fabs(U_R[i][j].phi_s-U_L[i][j].phi_s)<eps)
									...
									S = U_R[i][j].phi_s*P_l_MIDy[i][j+1]-U_L[i][j].phi_s*P_l_MIDy[i][j];
								...
								E_gC[i][j]    -= tau*(E_F_gy[i+1][j]  -E_F_gy[i][j])  /dy-tau/dx*S*U_L[i][j].u_s;
								...
								E_lC[i][j]    -= tau*(E_F_ly[i+1][j]  -E_F_ly[i][j])  /dy+tau/dx*S*U_L[i][j].u_s;
								area_L=0.5+U_L[i][j].v_s*tau/dx;
								area_R=1.0-area_L;
								U[0] = ZRHO_gC[i][j];
								U[1] = RHO_V_gC[i][j];
								U[2] = RHO_U_gC[i][j];
