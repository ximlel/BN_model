...
static void slope_limiter_GRP(int n_x, double Z_lC[][n_x], double Z_I_lxL[][n_x+1], double Z_I_lxR[][n_x+1], double Z_I_lyL[][n_x+1], double Z_I_lyR[][n_x+1], double Z_lx[][n_x], double Z_ly[][n_x],
							  ...
							  double RHO_lC[][n_x], double P_lC[][n_x], double U_lC[][n_x], double V_lC[][n_x], double RHO_I_lxL[][n_x+1], double RHO_I_lxR[][n_x+1], double P_I_lxL[][n_x+1], double P_I_lxR[][n_x+1], double U_I_lxL[][n_x+1], double U_I_lxR[][n_x+1], double V_I_lxL[][n_x+1], double V_I_lxR[][n_x+1], double RHO_I_lyL[][n_x+1], double RHO_I_lyR[][n_x+1], double P_I_lyL[][n_x+1], double P_I_lyR[][n_x+1], double U_I_lyL[][n_x+1], double U_I_lyR[][n_x+1], double V_I_lyL[][n_x+1], double V_I_lyR[][n_x+1], double RHO_lx[][n_x], double P_lx[][n_x], double U_lx[][n_x], double V_lx[][n_x], double RHO_ly[][n_x], double P_ly[][n_x], double U_ly[][n_x], double V_ly[][n_x])
{
	...
	for(int i = 0; i < n_y; ++i)
		for(int j = 0; j < n_x; ++j)
			{
				Z_lx[i][j]   = (Z_I_lxL[i][j+1]   - Z_I_lxR[i][j])  /config[10];
				Z_ly[i][j]   = (Z_I_lyL[i+1][j]   - Z_I_lyR[i][j])  /config[11];
...
void finite_volume_scheme_GRP2D(struct flu_var * FV, const struct mesh_var * mv, const char * scheme, const char * problem)
{
	...
	double S, S_tmp, area_L, area_R, RHO_l_cell;
			...
			if (t_all >= plot_t)
				{
					for(int i = 0; i < n_y; ++i)
						for(j = 0; j < n_x; ++j)
							{
								FV->RHO[i*n_x+j]=ZRHO_lC[i][j]/Z_lC[i][j];
						...
						for(j = 1; j < n_x; ++j)
							{
								...
								phi_sL[i][j] = 0.5*(Z_lC[i-1][j-1]+Z_lC[i][j-1]);
								phi_sR[i][j] = 0.5*(Z_lC[i-1][j]+Z_lC[i][j]);					
						...
						for(j = 1; j < n_x; ++j)
							{
								...
								phi_sL[i][j] = 0.5*(Z_lC[i-1][j-1]+Z_lC[i-1][j]);
								phi_sR[i][j] = 0.5*(Z_lC[i][j-1]+Z_lC[i][j]);					
								primitive_comp(U, &U_L[i][j], &U_R[i][j], phi_sL[i][j], phi_sR[i][j], 0.5, 0.5);
								RHO_gC[i][j] = U_L[i][j].lo_g;
			...
			if (stop_step == 0)
				{									
					...
					for(int i = 1; i < n_y; ++i)
						for(j = 1; j < n_x; ++j)
							{
								RHO_l_cell  = 0.25*(RHO_lC[i][j]+RHO_lC[i+1][j]+RHO_lC[i][j+1]+RHO_lC[i+1][j+1]);
								RHO_l_cell -=tau*(stag_ZRHO_F_lx[i][j+1]-stag_ZRHO_F_lx[i][j])/dx;
								ZRHO_lC[i][j] -=tau*(stag_ZRHO_F_lx[i][j+1]-stag_ZRHO_F_lx[i][j])/dx;
								Z_lC[i][j]     =ZRHO_lC[i][j]/RHO_l_cell;
							}
					for(int i = 1; i < n_y; ++i)
						for(j = 1; j < n_x; ++j)
							{
			...
			else if (stop_step == 2)
				{
						...
						for(j = 1; j <=n_x; ++j)
							{
								z_lyL = Z_ly[i][jL];
								z_lyR = Z_ly[i][jR];
								z_lL =Z_lC[i][jL]+dy/2*z_lyL;
								z_lR =Z_lC[i][jR]-dy/2*z_lyR;
					...
					for(int i = 1; i < n_y; ++i)
						for(j = 1; j < n_x; ++j)
							{
								RHO_l_cell  = 0.25*(RHO_lC[i][j]+RHO_lC[i+1][j]+RHO_lC[i][j+1]+RHO_lC[i+1][j+1]);
								RHO_l_cell -=tau*(stag_ZRHO_F_ly[i+1][j]-stag_ZRHO_F_ly[i][j])/dx;
								ZRHO_lC[i][j] -=tau*(stag_ZRHO_F_ly[i+1][j]-stag_ZRHO_F_ly[i][j])/dy;
								Z_lC[i][j]     =ZRHO_lC[i][j]/RHO_l_cell;
							}
					for(int i = 1; i < n_y; ++i)
						for(j = 1; j < n_x; ++j)
						...
						for(j = 1; j < n_x; ++j)
							{
								if(fabs(U_R[i][j].phi_s-U_L[i][j].phi_s)<eps)
									...
									S = U_R[i][j].phi_s*P_g_MIDy[i][j+1]-U_L[i][j].phi_s*P_g_MIDy[i][j];
	...
	for(int i = 0; i < n_y; ++i)
		for(j = 0; j < n_x; ++j)
			{
				FV->RHO[i*n_x+j]=ZRHO_gC[i][j]/Z_lC[i][j];
				FV->Z_a[i*n_x+j]=Z_lC[i][j];
				FV->PHI[i*n_x+j]=Z_lC[i][j];
			}
