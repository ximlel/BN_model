				...
				for(j = 0; j < n_x-1; ++j)
					{
						U[0] = ZRHO_gC[i][j];
						U[1] = RHO_U_gC[i][j];
						U[2] = E_gC[i][j]-0.5*RHO_V_gC[i][j]*RHO_V_gC[i][j]/ZRHO_gC[i][j];
						U[4] = ZRHO_lC[i][j];
						U[5] = RHO_U_lC[i][j];
						U[6] = E_lC[i][j]-0.5*RHO_V_lC[i][j]*RHO_V_lC[i][j]/ZRHO_lC[i][j];			
						phi_sL[i][j] = 0.5*(Z_gC[i][j]+Z_gC[i+1][j]);
						phi_sR[i][j] = 0.5*(Z_gC[i][j+1]+Z_gC[i+1][j+1]);					
